...
> ## 🎯 캐릭터 + 제품 광고 릴스를 반복 생성하려면 → [PIPELINE.md](PIPELINE.md)
> **캐릭터 이미지 + 제품 사진**을 넣어 "문제→제품 등장→사용→완벽한 결과" 구조의
> 광고 릴스를 AI로 만드는 워크플로우입니다. (Higgsfield 크레딧 필요)
> 자막·음성은 영상마다 **한국어/영어** 선택 가능.
> 관련 파일: `briefs/`, `plan_reel.py`, `compose_reel.py`, `reel/brief.py`, `reel/compose.py`
>
> 아래 내용은 AI 없이 **정적 이미지 + 자막**으로 만드는 무료 기본 모드입니다.
