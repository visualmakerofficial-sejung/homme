...
export function BidCalculatorPanel({ open, onClose }: Props) {
  ...
  return (
    <CalcPanel open={open} onClose={onClose} title="입찰계산기" icon={Banknote}>
      ...
      {result && (
        <ResultCard title="투자 분석 결과">
          ...
          <ResultHighlight
            label="순수이익"
            value={`${result.netProfit >= 0 ? "+" : ""}${man(result.netProfit)}`}
