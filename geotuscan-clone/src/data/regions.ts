 ...
 * 시도 / 시군구 목록.
 *
 * 원본은 시도를 고르면 API가 시군구를 내려주는 구조로 보이지만 스냅샷에는
 * 응답이 없다. 셀렉트가 실제로 동작하도록 표준 행정구역을 정적으로 넣었다.
 * 원본 API를 붙일 때는 이 파일만 교체하면 된다.
 */
...
/** 통계 화면의 시작월/종료월 옵션. 원본 부제가 "(2024~)"라 2024-01부터. */
export function monthOptions(from = "2024-01"): string[] {
