...
| 입찰계산기 (헤더) | 전체화면 오버레이 | 입력부 복제, 결과부는 추정 |
| 아파트 계산기 · DSR (헤더) | — | **스냅샷 없음** → 공개 산식으로 재구성 |
...
| 입찰계산기 **결과 영역** | 금액칸이 비어 있어 결과가 렌더 안 됨 → 하단 면책 문구(종합소득세·일반세율)를 근거로 구성. **원본과 항목·산식이 같다는 보장 없음** |
| 아파트 계산기 / DSR | 스냅샷 없음 → 공개 산식으로 재구성, 실제 동작함 |
...
└─ features/
   ...
   └─ ComingSoonPage.tsx       (현재 미사용 — 모든 탭이 구현돼 참조 없음)
```
...
- 입찰계산기 전체화면 오버레이 열림 → 기본값(300/80/4.5) → 결과 계산 → 닫힘
