...
- 지역 3단 셀렉트는 **올린 파일에 실제로 있는 지역**으로 채운다 (없는 지역을
  고를 수 있어 봐야 결과가 비기 때문)
- 지표 탭 4종이 피벗의 열이 된다: 층수 / 건축년도 / 전용평수 / 매매금액
- 행은 연도, 각 행에 중위 거래가를 같이 낸다
- "거래량 TOP 20 동네"도 올린 데이터로 실제 집계한다. 시군구까지 좁히면
  읍면동 단위로 센다
