...
| 빌라데이터 | `/villa-data` | 복제 완료 + **실거래가 파일로 실제 동작** |
...
| 아파트 계산기 · DSR (헤더) | — | **제외** — 스냅샷이 없어 산식을 지어내야 해서 뺐다 |
...
빌라데이터 탭은 실제로 동작한다. 국토교통부 실거래가 공개시스템에서 받은
파일을 올리면 브라우저가 파싱해 **연도별 거래건수 피벗**을 그린다.

- 받는 곳: <https://rt.molit.go.kr/pt/xls/xls.do?mobileAt=> (페이지 안에 링크 있음)
- 지원 형식: CSV · XLS · XLSX
- 인코딩: MOLIT CSV는 보통 CP949다. UTF-8로 먼저 읽고 실패하면 euc-kr로 다시 읽는다
- 로직은 `src/lib/molit.ts` 에 몰아 뒀다 (파싱 · 피벗 · TOP 20 · 지역 추출)

### 왜 API 호출이 아니라 파일 업로드인가

`xls.do`는 공개 API가 아니라 그 사이트에서 조건을 고르고 받는 **다운로드
엔드포인트**다. 세션·폼 파라미터에 묶여 있고 CORS도 열려 있지 않아 브라우저에서
직접 부를 수 없다. 게다가 아티팩트 호스팅은 CSP로 외부 fetch 자체를 막는다.
그래서 받은 파일을 읽는 방식으로 붙였다. **파일은 브라우저 안에서만 처리되고
서버로 전송되지 않는다.**

서버를 둘 수 있다면 공공데이터포털(data.go.kr)의 실거래가 API를 서버에서
호출해 캐시하는 쪽이 낫다. 그때는 `molit.ts`의 `Deal[]`만 채워 주면
피벗·TOP 20은 그대로 돌아간다.
...
거래를 한 줄씩 늘어놓지 않는다. **구간별로 몇 건인지** 세서 피벗으로 준다.

- 지역 3단 셀렉트(시도·시군구·읍면동)는 **올린 파일에 실제로 있는 지역**으로
  채운다. 없는 지역을 고를 수 있어 봐야 결과가 비기 때문이다.
- 탭 4종은 **집계 기준**이다. 구간은 원본 화면과 같게 맞췄다.

  | 탭 | 구간 |
  |---|---|
  | 층수 | 지하층 · 1층 · 2층 · 3층 · 4층 · 5층+ |
  | 건축년도 | ~5년 · 5~10 · 10~15 · 15~20 · 20~25 · 25~30 · 30년+ (계약연도 − 건축년도) |
  | 전용평수 | ~5평 · 5~10 · 10~15 · 15~20 · 20~25 · 25~30 · 30평+ |
  | 매매금액 | ~5천 · 5천~1억 · 1~1.5 · 1.5~2 · 2~2.5 · 2.5~3 · 3~4 · 4~5 · 5~6 · 6억+ |

- 행은 연도, 열은 구간. 오른쪽에 합계 열, 아래에 합계 행.
- "카테고리별 보기"를 누르면 행·열이 뒤집힌다 (구간이 행, 연도가 열).
- 셀 값은 표 안 최댓값 기준으로 강·약을 준다. 85% 이상은 주황, 30% 이하는 흐린 파랑.
- **합계 열은 그 해 거래건수**이고 구간 합이 아니다. 건축년도·전용면적이 비어 있는
  거래는 어느 구간에도 넣지 않기 때문에 건축년도 탭에서는 둘이 다를 수 있다.
  (원본 화면도 같게 동작한다. 도넛 아래에 제외 건수를 적어 둔다.)

**그래프 2종** — `src/features/villa-data/charts.tsx`, 외부 차트 라이브러리 없이 SVG로 직접 그린다.

- **연도별 {지표} 구성**: 연도마다 막대 하나, 안이 구간별로 쌓인다. 칸 사이 2px.
  y축은 최댓값을 한 단계 올림해 4등분한다 (원본의 `0 / 8K / 15K / 23K / 30K`).
- **{지표}별 비중**: 도넛. 조각에 올리면 가운데에 구간·건수·비율이 뜬다.
- 색은 원본 화면 순서 그대로(파랑·초록·주황·빨강·보라·청록·오렌지·분홍·하늘·올리브).
  초록만 `#22c55e → #2e9e4f`로 한 단계 눌렀다 — 원래 값은 적록색약에서 주황과
  ΔE 5.7까지 붙어 구분이 안 된다(지금 8.6). 자세한 건 `charts.tsx` 위 주석에.

**거래량 TOP 20 동네**

- 버튼을 누르면 **서울 / 경기 / 인천 / 그 외** 4개 메뉴가 뜬다.
  파일에 자료가 없는 지역군은 흐리게 비활성된다.
- 각 지역군 안에서 **동별** 거래건수 1위부터 20위까지. 중위가도 같이 낸다.
- 동 이름은 구를 건너 겹치므로(예: 중동) `시군구 + 동`으로 묶어 센다.
- 줄을 누르면 그 동네로 지역 셀렉트가 맞춰진다.
...
현재 팔레트는 shadcn 뉴트럴 베이스 + 블루 프라이머리
(`oklch(0.546 0.198 262.9)` ≈ `#2563eb`)다. **이대로 확정됐다.**

원본 팔레트는 복원하지 못했다. 저장된 HTML은 CSS를 외부 파일
(`34d933785a17edf3.css`, `d34f76c7183e71bc.css`)로 참조만 하고 있고, 그 파일들은
업로드된 스냅샷에 포함돼 있지 않다. 마크업에는 `bg-primary`,
`text-muted-foreground` 같은 **토큰 이름만** 있고 raw 값이 하나도 없다.
즉 지금 색은 원본을 맞춘 게 아니라 새로 고른 값이다.
...
| 시도·시군구 목록 | 원본은 API로 받는 듯 → 표준 행정구역을 정적으로 넣음 (`src/data/regions.ts`). 단 빌라데이터는 올린 파일에서 실제 지역을 뽑아 쓴다 |
...
| 아파트 계산기 / DSR | 스냅샷이 없어 산식을 확인할 수 없었다. 지어낸 계산 결과를 보여주느니 빼는 편이 낫다고 보고 **헤더 버튼째 제외**했다. 원본 화면을 확보하면 되살린다 |
...
1. **헤더의 "아파트 계산기"·"DSR" 버튼을 뺐다.** 원본에는 있지만 두 화면의
   스냅샷을 못 받아 내부 구성과 산식을 확인할 수 없었다. 근거 없는 계산 결과를
   보여주는 것보다 없는 편이 낫다고 판단했다. 원본 화면을 확보하면
   `Header.tsx`에 버튼과 패널을 되살리면 된다 (`calc-ui.tsx`의 조각을 그대로 쓸 수 있다).
2. **최근 검색 삭제 버튼에 `[@media(hover:none)]:opacity-100`을 더했다.**
   터치 기기에는 hover가 없어 원본 그대로면 버튼이 영영 안 보인다.
...
입찰계산기는 `components/dialogs/calc-ui.tsx`의 셸(`CalcPanel` / `CalcSection` /
`MoneyRow` / `ResultCard` …)을 쓴다. 계산기가 더 생기면 같은 조각을 재사용하면 된다.
